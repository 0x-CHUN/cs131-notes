...
def mix_quadrants(image):
    ...
    tl_br = rgb_exclusion(image,'R')
    tr = dim_image(image)
    bl = np.sqrt(image)
    
    out[:(image.shape[1])//2,:(image.shape[1])//2,:]=tl_br[:(image.shape[1])//2,:(image.shape[1])//2,:]
    out[(image.shape[1])//2:,(image.shape[1])//2:,:]=tl_br[(image.shape[1])//2:,(image.shape[1])//2:,:]
    out[:(image.shape[1])//2,(image.shape[1])//2:,:]= tr[:(image.shape[1])//2,(image.shape[1])//2:,:]
    out[(image.shape[1])//2:,:(image.shape[1])//2,:]=bl[(image.shape[1])//2:,:(image.shape[1])//2,:]
    # END YOUR CODE
